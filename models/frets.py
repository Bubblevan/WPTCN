...
class BackboneFreTS(nn.Module):
    ...
    def forward(self, x):
        # x 的形状为 (batch_size, n_steps, embed_size)
        B, T, N = x.shape
        x = x.permute(0, 2, 1)  # 调整为 (batch_size, embed_size, n_steps)
        x = x.unsqueeze(3)  # 变为 (batch_size, embed_size, n_steps, 1)
        bias = x
        # 频域操作
        ...
        x = x + bias
        x = x.reshape(B, -1)  # 展平为 (batch_size, embed_size * n_steps)
        # print(f"x shape before fc: {x.shape}")
        x = self.fc(x)  # 输出形状为 (batch_size, n_classes)
